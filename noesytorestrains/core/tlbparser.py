...
class TblFileMaker:
    ...
    def __init__(
        ...
        self.forcefield_types = {
            "ALA": ["HN", "HA", "HB1rot", "HB2rot", "HB3rot"],
            "ARG": [
                "HN",
                "HA",
                "HB1",
        ...
        self.basestring = (
            "assign (resid {0} and name {1})(resid {2} and name {3}) {4} {5} {6}"
        )
            ...
            for line in f:
                for aa in line.strip().split(" "):
                    if any((aa == "", aa == " ", aa is None)):
                        raise ValueError(
                            "The file does have a" + "bad structure, check it"
                        )
    ...
    def read_noecsv(self):
        with open(self.noecsv_fname, "r") as csvfile:
            ...
            for i, row in enumerate(reader):
                ...
                self.parameters_to_save.append(
                    (
                        int(matched.group(1)),
                        matched.group(2),
    ...
    def atom_to_forcefield(self, atom_name=None, aminoacid=None, atom_num=None):
        ...
        for ff_type_for_atom in ff_types_for_aa:
            ...
            if atom_num is not None and ff_atom_num is not None:
                if atom_num != ff_atom_num and len(str(atom_num)) == len(
                    str(ff_atom_num)
                ):
                    continue
            ...
            if ff_atom_name == atom_name:
                if ff_atom_num is not None:
                    atom_names_list.append(
                        (ff_atom_name + str(ff_atom_num), rotational)
                    )
                else:
    ...
    def distance_restrain_pairs(self, entry_pair=None):
        ...
        atom1_f = self.atom_to_forcefield(
            atom_name=atom1,
            atom_num=atomnum1,
            aminoacid=self.aminoacid_sequence[res1 - 1],
        )
        atom2_f = self.atom_to_forcefield(
            atom_name=atom2,
            atom_num=atomnum2,
        ...
        atom_product, atom_product_tee = tee(
            product(atom1_f_names_only, atom2_f_names_only)
        )
        if at_least(self.one_atom_max_limit + 1, atom_product_tee):
            print(
                "possible pairs for aminoacids {0} and {1} exceeding {2}".format(
                    res1, res2, self.one_atom_max_limit
                )
            )
            return None
    ...
    def save_tbl(self, tbl_dir="tblfiles"):
        ...
        print(
            "Generating different combinations for files to save, if it is too long, kill the app"
        )
        ...
        for i, protein_restrain in enumerate(protein_restrains):
            ...
            with open(
                "./" + tbl_dir + "/" + str(i) + "_" + self.restwrite_fname, "w"
            ) as f:
...
if __name__ == "__main__":
    tblmaker = TblFileMaker(
        noecsv_fname="noe.csv",
        restwrite_fname="protein.tbl",
        sequence_fname="sequence.seq",
    )
